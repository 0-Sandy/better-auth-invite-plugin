...
import { INVITE_COOKIE_NAME } from "../src/constants";
...
test("activateInvite uses custom cookie names", async ({ createAuth }) => {
	const { client, signInWithTestUser, auth, db } = await createAuth({
		pluginOptions: {
			...defaultOptions,
			canCreateInvite: () => true,
		},
	...
	const { error, data } = await client.invite.activate({
		...
		fetchOptions: {
			async onResponse(context) {
				...
				const sessionCookie = (await auth.$context).createAuthCookie(
					INVITE_COOKIE_NAME,
				);
				const signedCookie = cookies.get(sessionCookie.name)?.value;
				newHeaders.set("cookie", `${sessionCookie.name}=${signedCookie}`);
			},
		},
	});
